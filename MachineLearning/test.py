...
def test_func(x):
    ...
    if date.tm_hour==10:
        ...
        if date.tm_min == 00:
            ...
            query = '''UPDATE RoadA set `02:00-02:15`=%s WHERE Id=%s'''
        elif date.tm_min==30:
            query = '''UPDATE RoadA set `02:15-02:30`=%s WHERE Id=%s'''
        elif date.tm_min == 45:
            query = '''UPDATE RoadA set `02:30-02:45`=%s WHERE Id=%s'''
        else:
            pass

    elif date.tm_hour==15:
        if date.tm_min == 00:
            query = '''UPDATE RoadA set `02:45-03:00`=%s WHERE Id=%s'''
        else:
            pass
    ...
    elif (date.tm_min==15 or date.tm_min==30 or date.tm_min==45 or date.tm_min==00) and (date.tm_hour==11 or
                                                                                               date.tm_hour==12 or
                                                                                               date.tm_hour==13 or
                                                                                               date.tm_hour==14):
    ...
    try:
        ...
        print ("Not the time to entry data!")
    finally:
        db.commit()
        db.close()
